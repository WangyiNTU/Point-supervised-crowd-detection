...
def nn_p3p4p5_up(img_input=None, offset=True, num_scale=1, trainable=False):
    bn_axis = 3
    x = ZeroPadding2D((3, 3))(img_input)
    x = Convolution2D(64, (7, 7), strides=(2, 2), name='conv1', trainable=False)(x)
    x = BatchNormalization(axis=bn_axis, name='bn_conv1')(x)
    ...
    # upsize to input
    feat = Deconvolution2D(16, kernel_size=4, strides=2, padding='same',
                           kernel_initializer='glorot_normal', name='P_up_2', trainable=trainable)(feat)

    feat = Convolution2D(16, (3, 3), padding='same', kernel_initializer='glorot_normal', name='feat_up_2',
                         trainable=trainable)(feat)
