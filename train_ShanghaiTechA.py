...
if C.offset:
    C.path_history = None#''output/valmodels/ShanghaiTechA/%s/off' % (C.scale)
else:
    C.path_history = None#'output/valmodels/ShanghaiTechA/%s/nooff' % (C.scale)

C.update_thr = 0.6
C.update_weights_thr = 10
C.alpha = 1
# get the training data
cache_path = 'data/cache/ShanghaiTechA/train'
cache_path_updated = 'data/cache/ShanghaiTechA/train_updated'
...
if C.path_history is not None:
    ...
    if C.offset:
        out_path = 'output/valmodels/ShanghaiTechA/%s/off' % (C.scale)
    else:
        out_path = 'output/valmodels/ShanghaiTechA/%s/nooff' % (C.scale)
...
### for validation
cache_path_val = 'data/cache/ShanghaiTechA/train_val'
with open(cache_path_val, 'rb') as fid:
    val_data = cPickle.load(fid)

cache_path_test = 'data/cache/ShanghaiTechA/test'
